...
| MAE | 0.6954 | measured |
| MAPE | 0.82% | measured |
| Forecasting accuracy on Telecom Italia MI | `<TO MEASURE>` (Linux/storage-deferred) | Plan: (1) download the Telecom Italia "Big Data Challenge — Milan" SMS/Call/Internet dataset from its public archive (~tens of GB; one daily file is ~150 MB and is enough for an initial benchmark); (2) place file(s) under `data/telecom_italia_mi/`; (3) run `make run-telecom` (loader + hourly aggregation already implemented per `DATA_CONTRACT.md`); (4) capture RMSE/MAE/MAPE into `reports/forecast_examples/telecom_italia/metrics.json`. Deferred from the current Windows dev environment — best handled on the Linux dev/host once that environment is online tomorrow. |
...
| End-to-end forecast wall-clock (s) | 19.7 (Windows · Python 3.13.13) | measured ([reports/forecast_examples/latest/timing.json](reports/forecast_examples/latest/timing.json)) — machine-dependent; reproduce by timing `make run-sample` on your machine |
